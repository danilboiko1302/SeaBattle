...
public class GridCreator extends JPanel {
    ...
    private static boolean currentlyPlacingShip = false;

    GridCreator(Ship[] shipArray, int gridSize, JFrame app) {
        this(shipArray, gridSize, "gridLabels.png", app);
    }

    private GridCreator(Ship[] shipArray, int gridSize, String path, JFrame app) {
        setLayout(null);
     ...
     */
    void setup(boolean com) {
        System.out.println("zashlo");
        int largestShipSize = 0;
        for (Ship ship : shipArray) {
            int temp = ship.getShipPieces().length;
        ...
        randomizeShipsBtn.addActionListener(e -> {
            ...
            for (int i = 0; i < panelArray.length; i++) {
                ...
                while (timeout < 500
                    ...
                    //   System.out.println(Arrays.toString(shipArray));
...
//                    System.out.println(panelArray[shipNum].getX());
//                    System.out.println(panelArray[shipNum].getY());
                } else {
    ...
    private boolean isIntersection(JPanel p) {
        ...
        for (JPanel jPanel : panelArray) {
            ...
            // itself
            if (!p.equals(jPanel)) {
                if ((p.getX() <= jPanel.getWidth() + jPanel.getX() + 53 && p.getX() >= jPanel.getX() - 53) || (p.getX() + p.getWidth() <= jPanel.getWidth() + jPanel.getX() + 53 && p.getX() + p.getWidth() >= jPanel.getX() - 53)) {
                    if ((p.getY() <= jPanel.getHeight() + jPanel.getY() + 53 && p.getY() >= jPanel.getY() - 53) || (p.getY() + p.getHeight() <= jPanel.getHeight() + jPanel.getY() + 53 && p.getY() + p.getHeight() >= jPanel.getY() - 53)) {
                        return true;
                    }

                }
            }
            if (p.getBounds().intersects(jPanel.getBounds()) && !p.equals(jPanel)) {
                return true;
            }
        }
